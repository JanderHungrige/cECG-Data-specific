function   synch_after_intellivue(bin,intelleviue_save, mov,cam,bin_save,dataSetpath,datacreationPath)
% This version uses the NAME OF THE FILE  instead the file changed timestamp which is used in the second version
% and instead of the tdms information which is used in the first version. 
% There was a huge synchronisation error due to both
...
if str=='y' 
    ...
    Fortfahren=0;
    if lagDiff >=0
    ECG_bin_synched = ECG_bin_tot_500(lagDiff:end);
    t2 = (0:length(ECG_intelleview_synched)-1)/500;
    elseif lagDiff<0
    add(1:-lagDiff+1)=nan;
    ECG_bin_synched = [add,ECG_bin_tot_500];
    end
    savegappdiff=1;
elseif str=='o'
    Fortfahren=1;
...
% The data is extended or cut always at the DAQ ECG (FOr patients 15-18 the Video was linked with the intellivue signal)
    if Fortfahren==1
        Gapdiff=Gapstart_intelleview_idx-Gapstart_bin_idx; % getting data either from manual or gap find loop
        if Gapdiff >0 %intelleview gap is earlier
            %add zeroes or nans at the beginning until gapstart ==
            pasting(1:Gapdiff,1)=NaN;
            ECG_bin_synched=[pasting;ECG_bin_tot_500];
            EDR_bin_synched=[pasting;EDR_bin];
        elseif Gapdiff <0 % Intelleview Gap is later
            %Cut intelleview form the begining as it starts earlier than bin
            ECG_bin_synched=ECG_bin_tot_500(abs(Gapdiff):end,1);
            EDR_bin_synched=EDR_bin(abs(Gapdiff):end,1);
        elseif Gapdiff == 0
...
%         disp(['The difference between gap_bin and gab_intelle start is ', num2str(Gapdiff), ' samples and ', num2str(Gapdiff/500), ' seconds' ])
    ...
    for j=1:1 % Plotting; for loop just to be able to fold the loop together to save space
       ...
       subplot (3,1,1)
        ...
        title('bin ECG')
        xlim([0 length(ECG_intelleview_tot)])
        
       subplot(3,1,2)
        plot(ECG_bin_synched);hold on
        title('sync bin ECG ')
        xlim([0 length(ECG_intelleview_tot)])
...
%% ************** Create Intellivue REspiration signal (formals synchronisation of ECG and Resp )**************
...
%% ******************* start Differnt part to version 1 **********************
%GETTING TIMESTAMP OIF INTELLIVUE  
cd(dataSetpath{2})
%--------------ECG-----------
ECG_intelleview_files=dir('II_*.txt'); % find all REf ECG files
kb=ECG_intelleview_files.bytes;
...
numFiles=size(ECG_intelleview_files,1);%amount of ECG ref files 
%read in txt files
    filename=ECG_intelleview_files(1).name;
    delimiter = '\t';
    startRow = 15; % first few lines are nan. Check if always 15
    ...
    tline = fgetl(fileID);
        if i==4
           timestamp_intellivue=tline; 
           timestamp_intellivue=split(timestamp_intellivue, '"');timestamp_intellivue=timestamp_intellivue{2,1};
           clearvars tline
        end
    end
    DATAonlydaetime = (datetime(timestamp_intellivue,'Format','MM/dd/yyyy hh:mm:ss', 'ConvertFrom', 'datenum')); % create datetime for easier manipulation https://stackoverflow.com/questions/44158808/time-stamp-conversion-to-seconds#
    timeStartIntellivue = datenum(DATAonlydaetime); % days since January 0, 0000
...
if isempty(videonames)==1
    disp('No video for that file found')
    return
end
% &&&&& loading camera start time from Video name
    %create video filename from TDMS filename 
    CAMonlydaetime=strsplit(videonames.name,'_');CAMonlydaetime=CAMonlydaetime{1,1};% now y-MM-dd hh.mm.ss
    % CAMonlytime=strread(CAMonlydaetime,'%s','delimiter',' ');onlytime=onlytime{2,1}; onlytime=strrep(onlytime,'.',':');
    CAMonlydaetime = (datetime(CAMonlydaetime,'Format','yyyy-MM-dd hh.mm.ss', 'ConvertFrom', 'datenum')); % create datetime for easier manipulation https://stackoverflow.com/questions/44158808/time-stamp-conversion-to-seconds#
    timeStartCam = datenum(CAMonlydaetime); % days since January 0, 0000


   offsetCam_s=(timeStartCam-timeStartIntellivue)*24*3600; % Difference in [s]
   
   %Get he info also in hh:mm:ss
    Stunden=offsetCam_s/3600;[Stunden,Minuten]=deal(fix(Stunden), Stunden-fix(Stunden));
    Minuten=Minuten*60;[Minuten,Sekunden]=deal(fix(Minuten), Minuten-fix(Minuten));
    Sekunden=fix(Sekunden*60);

%% ******************* END Differnt part to version 1 **********************
...
disp(['Camera ECG offset is: ' num2str(Stunden) ':' num2str(Minuten) ':' num2str(Sekunden)  ' [hh:mm:ss]'])
...
%     addition(1:abs(offsetCAM_500),1)=NaN;
%     ECG_bin_synched          =[addition'  ECG_bin_synched']';
%     Resp_bin_synched         =[addition'  Resp_bin_tot_500']';
%     EDR_bin_synched          =[addition'  EDR_bin_synched']';
%     Resp_intelleview_synched =[addition'  Resp_intelleview_tot_500']';
%     EDR_intelleview_synched  =[addition'  EDR_intelleview']';
%     ECG_intelleview_synched  =[addition'  ECG_intelleview_tot']';
% elseif offsetCam_s >0 %camera started earlier than ECG
%     %cut the first few samples of ECG, intelleview ECG, Respiration
%     ECG_bin_synched          =ECG_bin_synched(abs(offsetCAM_500):end,1);
%     Resp_bin_synched         =Resp_bin_tot_500(abs(offsetCAM_500):end,1);
%     EDR_bin_synched          =EDR_bin_synched(abs(offsetCAM_500):end,1);
%     Resp_intelleview_synched =Resp_intelleview_tot_500(abs(offsetCAM_500):end,1);
%     EDR_intelleview_synched  =EDR_intelleview(abs(offsetCAM_500):end,1);
%     ECG_intelleview_synched  =ECG_intelleview_tot(abs(offsetCAM_500):end,1);
% elseif offsetCam_s==0
%     ECG_intelleview_synched=ECG_intelleview_tot_500;
...
% end
    ...
    ECG_bin_synched          =[ECG_bin_synched']';
    Resp_bin_synched         =[Resp_bin_tot_500']';
    EDR_bin_synched          =[EDR_bin_synched']';
    Resp_intelleview_synched =[Resp_intelleview_tot_500']';
    EDR_intelleview_synched  =[EDR_intelleview']';
