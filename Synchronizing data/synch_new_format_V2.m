function   synch_new_format_V2(bin,intelleviue_save, mov,cam,bin_save,dataSetpath,datacreationPath)
% This version uses the time created timestamp instead of the tdms
% information which is used in the first version. 
% There was a huge synchronisation error due to that
...
% get creation date of Video TDMS file (Video files are changed, therfore false date)
DateOfFileCreation=videonames.date;
DateOfFileCreation = (datetime(DateOfFileCreation, 'ConvertFrom', 'datenum')); % create datetime for easier manipulation https://stackoverflow.com/questions/44158808/time-stamp-conversion-to-seconds#

%create video filename from TDMS filename to find matching files
onlydaetime=strsplit(videonames.name,'_');onlydaetime=onlydaetime{1,1};% now y-MM-dd hh.mm.ss
onlydaetime=regexprep(char(onlydaetime), '[^0-9]',''); % SPACE is just a space ;-); deleting stuff out of string :https://nl.mathworks.com/matlabcentral/answers/82362-deleting-part-of-a-string
onlydaetime=insertAfter(onlydaetime,8,'_'); 

% Load duration of video whith the same name as the TDMS file to subtractthat from the creation time stamp. As the file was created after finishing the video
video_preview_folder= dir([ dataSetpath{4} '\*_video_preview']);
Videosfiles=dir([dataSetpath{4} '\' video_preview_folder.name '\' onlydaetime '.avi']);
vidlength=mmfileinfo([Videosfiles.folder '\' Videosfiles.name]);vidlength=vidlength.Duration; %get duration in seconds
Stunden=vidlength/3600;
[Stunden,Minuten]=deal(fix(Stunden), Stunden-fix(Stunden));
Minuten=Minuten*60;
[Minuten,Sekunden]=deal(fix(Minuten), Minuten-fix(Minuten));
Sekunden=fix(Sekunden*60);

% Subtract duration from creation date to get start date
DateOfFileCreation.Hour   = DateOfFileCreation.Hour - Stunden; % correct for the offset of 7 hours. ?timezone? format long
DateOfFileCreation.Minute = DateOfFileCreation.Minute - Minuten; % correct for the offset of 7 hours. ?timezone? format long
DateOfFileCreation.Second = DateOfFileCreation.Second - Sekunden;
timeStartCam = datenum(DateOfFileCreation); % create datetime for easier manipulation https://stackoverflow.com/questions/44158808/time-stamp-conversion-to-seconds#
...
Stunden_diff=offsetCam_s/3600;[Stunden_diff,Minuten_diff]=deal(fix(Stunden_diff), Stunden_diff-fix(Stunden_diff));
Minuten_diff=Minuten_diff*60;[Minuten_diff,Sekunden_diff]=deal(fix(Minuten_diff), Minuten_diff-fix(Minuten_diff));
Sekunden_diff=fix(Sekunden_diff*60);

%% ******************* END Differnt part to version 1 **********************
%%%%%% START Calcualtions from the TDMS file (NOT REALY USED)
        ...
        ticks2_CAM = int64(timeStartCam*1e9*24*36);%+7*3600; % convert to Ticks (100ns)
        dTicks = ticks2_CAM-ticks1_ECG;       % Difference in ticks (100 nanoseconds)
        offsetCam_s_TDMS = double(dTicks)*(1e-7);  % Difference in seconds. 10^-7 because of .Net System time which is accurate on 100 nano seconds. Therefore, second based is 10-7
%%%%%% END Calcualtions from the TDMS file (NOT REALY USED)
Unterschied=offsetCam_s_TDMS-offsetCam_s;
Stunden=Unterschied/3600;
[Stunden,Minuten]=deal(fix(Stunden), Stunden-fix(Stunden));
Minuten=Minuten*60;
[Minuten,Sekunden]=deal(fix(Minuten), Minuten-fix(Minuten));
Sekunden=fix(Sekunden*60);
disp(['difference TDMS - change of file(-duration) is: ' num2str(Stunden) ':' num2str(Minuten) ':' num2str(Sekunden)  ' [hh:mm:ss]'])
...
disp(['Camera ECG offset is: ' num2str(Stunden_diff) ':' num2str(Minuten_diff) ':' num2str(Sekunden_diff)  ' [hh:mm:ss]'])
