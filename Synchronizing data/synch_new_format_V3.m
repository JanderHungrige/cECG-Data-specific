function   synch_new_format_V3(bin,intelleviue_save, mov,cam,bin_save,dataSetpath,datacreationPath)
...
disp('')
disp(['To compare: 500Hz intelleview ECG length is  ' num2str(length(ECG_intelleview_tot)-length(Resp_intelleview_tot_500))]);
...
disp(['To compare: 500Hz intelleview RESP ECG differnce in length is ' num2str(Resp_ECG_diff) '(' num2str(Resp_ECG_diff*500) '[s])']);
...
% &&&&& loading start time from DAQ data name
    proxname=regexprep(char(proxname), '[.]','');
    datafoldername=dir([dataSetpath{4} '*_' proxname '*']); % loading all Data folder name
    DATAonlydaetime = (datetime(datafoldername.name,'Format','yyyyMMdd_hhmmss', 'ConvertFrom', 'datenum')); % create datetime for easier manipulation https://stackoverflow.com/questions/44158808/time-stamp-conversion-to-seconds#
    timeStartECG = datenum(DATAonlydaetime); % days since January 0, 0000

   offsetCam_s=-(timeStartCam-timeStartECG)*24*3600; % Difference in [s]
