% Create cECG and Raw cECG Jans way. 
% Alines software crshes when there are more than 2 files. 
...
%     %%%%%% INSERTED BY JAN
            ...
            % calc total length of analog ECG from data length
                ...
                disp(['Ausgezaehlt: Size ECG (and RESP) file #' num2str(fileNumber+1) ': ' num2str(sizeFile) ' samples (' num2str(sizeFile/8000/60) ' minutes)']);
    %%%%%% INSERTED BY JAN
...
% ************** Read reference ECG ******************
        ...
        ECG1 = ECG_bin_cell{1,1};
        if numFiles > 1 % if there is more than one file
            if fileNumber+1>1 % from file 2 onwards
                for j=2:numFiles
                    ECG1=[ECG1, ECG_bin_cell{1,j}];
                end
            end
        end; clearvars  ECG_bin_data_cell
    %%%%%% INSERTED BY JAN start

        for fileNumber=0:numFiles-1
            for sens=1:8
                m = eval(['memmapfile(''Sensor ',num2str(sens),'_0000000',num2str(fileNumber),'.bin'',''format'',''uint8'');']);
                x=m.data; clearvars m;
                LengthData=length(x);
                eval(['SEN' num2str(sens) '_cell{' num2str(fileNumber+1) '} = double(typecast(x(1:3:LengthData),''int8'')'')*2^16+double(x(2:3:LengthData)'')*256+double(x(3:3:LengthData)'');']);

                 %Create one long cECG file
                eval(['SEN' num2str(sens) '=' 'SEN' num2str(sens) '_cell{1,1};' ]);
                if numFiles > 1
                     if fileNumber+1>1 % from file 2 onwards
                        for j=2:numFiles
                            eval(['SEN' num2str(sens) '=' '[SEN' num2str(sens) ',' 'SEN' num2str(sens) '_cell{1,' num2str(j) '}];']);
%                             eval(["SEN" num2str(sens) "= SEN" num2str(sens) "';"]);% invert 
                            
                        end
                     end
                end
                clearvars  SEN1_cell SEN2_cell SEN3_cell SEN4_cell SEN5_cell SEN6_cell SEN7_cell SEN8_cell
            end
        end
        
%       SEN1=SEN1';SEN2=SEN2';SEN3=SEN3';SEN4=SEN4';SEN5=SEN5';SEN6=SEN6';SEN7=SEN7';SEN8=SEN8';
