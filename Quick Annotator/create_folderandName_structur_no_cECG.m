...
function create_folderandName_structur_no_cECG(participant,drive,QAfolder,video)
...
if exist([drive 'cECG_study\For_Quick_Annotator\Annotations\Participant' num2str(participant)], 'file')==0
status = mkdir ([drive 'cECG_study\For_Quick_Annotator\Annotations\Participant' num2str(participant)]);
end
%% Create folders
%*******************************************************************
...
%*******************************************************************
 names=dir('2012*_1*'); % find all session folders
    ...
    if numFiles ~=0 %if there is at least one file
        for fileNumber=1:1 % CHANGED  1:numFiles %go through each session of particular participant
            if exist([drive 'cECG_study\' folder.name '\' names(fileNumber,1).name '\Synched Data'],'file')
                ...
                %Go to folder
                if exist([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant)],'file')==0
                    status = mkdir([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant)]);  
                end
                if exist([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\camera\' date],'file')==0
                    status = mkdir([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\camera\' date]);
                end
                if exist([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\DAQ\' date],'file')==0
                    status = mkdir([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\DAQ\' date]);
                end
                if exist([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\Intellivue\' date],'file')==0
                status = mkdir([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\Intellivue\' date]);
                end
    ...
    cd([drive 'cECG_study\' folder.name])
                ...
                cd([drive 'cECG_study\' folder.name '\' sessiondate_cell{fileNumber} '\Synched data']) % go to that folder
...
% create dummy cECG and motion filled with nans. Needed for the quick annotator
                    cap_ECG_synched     =nan(length(ECG_bin_synched),1);
                    motion_level_synched=nan(length(ECG_bin_synched),1);
        % Create new BIN variables
                    ...
                    save(([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\DAQ\' date '\' filename]), 'ECG', 'EDR','Resp', 'cECG', 'Motion','HRV');
                else
    ...
    % Create new Intelleview variables           
                    ...
                    % Store in new MAT-file
                    save(([drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\Intellivue\' date '\' filename]), 'ECG', 'EDR','Resp','HRV');
                 else
...
cd([drive 'cECG_study\' folder.name])
names=dir('*video*');
cd([drive 'cECG_study\' folder.name '\' names.name]) %go into videofolder
names=dir(['*.' videoformat]);
    ...
    if numFiles ~=0 %if there is at least one file
        for fileNumber=1:1 %changed 1:numFiles % go through each session of particular participant
            ...
            %Moving
            movefile( newnames.name ,[drive 'cECG_study\For_Quick_Annotator\participant' num2str(participant) '\camera\' date])
        end
    end
